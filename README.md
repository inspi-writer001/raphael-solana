...
Autonomous Solana + Polymarket agent wallet — installable as an OpenClaw skill.

Tell your agent: _"create a solana wallet on devnet/mainnet named sarah", "create an EVM wallet", "check my USDC balance", "start the weather arb scanner", "check status"_ — it handles everything on-chain, autonomously.
...
solana-wallet transfer matic <wallet> <to-address> <amount>
solana-wallet transfer erc20 <wallet> <to-address> <token-address> <amount>
...
src/
  ...
  evmTransfer.ts          MATIC + ERC-20 transfers on Polygon (returns txHash + Polygonscan URL)
